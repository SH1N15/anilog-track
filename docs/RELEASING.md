...
未签名 APK 通常位于 `android/app/build/outputs/apk/release/app-release-unsigned.apk`。使用 Android Studio 的 **Build → Generate Signed App Bundle or APK**，选择 APK，并使用项目发布密钥完成签名。
...
5. 上传两份 Windows 安装包和一份已签名 Android APK。
6. 标记为 Latest 并发布。
7. 发布后逐一检查三个下载链接、文件大小和 SHA-256。
